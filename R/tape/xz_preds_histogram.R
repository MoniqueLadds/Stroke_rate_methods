...
summary.xz<-apply(X = stroke[7:32],2,summary)
range.xz<-apply(stroke[7:32],2,function(x) max(x)-min(x))
median.xz<-apply(stroke[7:32],2,median)
mean.xz<-apply(X = stroke[7:32],2,mean)
sd.xz<-apply(X = stroke[7:32],2,sd)
ztest.xz<-apply(X = stroke[7:32],2,function(x) z.test(a=x,mu=0,var=15))

t.test(predictions$peak_1sec_50_dyn.xz,mu=0)

export.x<-cbind(mean.xz,sd.xz,median.xz,range.xz,ztest.xz)
write.csv(export.x,"output/peaks/summarystatsXZ.csv")
...
tiff("figs/tape/tape_xzhist.tiff",width = 600, height = 400)


#extract the data
plotData <- stroke$peak_0.4sec_30_dyn.xz 
par(mar=c(3.5,3.5,1,1))
h<-hist(plotData, breaks=10,border="white", xlab="",ylab = "",main="",
        ylim=c(0,30),xlim=c(-30,50)) 
text(50,30,"C",cex=1.5)
...
#lineData <- stroke$peak_0.4sec_10_dyn.xz
#density_lines(lineData, "mediumorchid1")

lineData <- stroke$peak_0.4sec_20_dyn.xz
density_lines(lineData, "hotpink3")

lineData <- stroke$peak_0.4sec_30_dyn.xz
density_lines(lineData, "darkorchid1")

#lineData <- stroke$peak_0.4sec_40_dyn.xz
#density_lines(lineData, "darkorchid1")

lineData <- stroke$peak_0.4sec_50_dyn.xz
